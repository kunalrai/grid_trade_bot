...
class GridTrader:
    """Implements the grid trading strategy for SOL/USDT"""

    def __init__(self, client: BinanceFuturesClient, config: Dict[str, Any], logger: TradingLogger):
        """
    ...
    def initialize(self) -> bool:
        ...
        """
        self.logger.info("=" * 60)
        self.logger.info("SOL/USDT GRID TRADING BOT - INITIALIZING")
        self.logger.info("=" * 60)
        ...
        # Set leverage
        if not self.client.set_leverage(self.symbol, self.leverage):
    ...
    def _execute_buy(self, current_price: float) -> bool:
        ...
        try:
            ...
            if self.order_type == 'market':
                order = self.client.place_market_order(self.symbol, 'buy', self.position_size)
                execution_price = current_price
            else:  # limit order
                order = self.client.place_limit_order(self.symbol, 'buy', self.position_size, self.buy_level)
    ...
    def _execute_sell(self, current_price: float) -> bool:
        ...
        try:
            # Place order
            if self.order_type == 'market':
                order = self.client.place_market_order(self.symbol, 'sell', self.position_size)
                execution_price = current_price
            else:  # limit order
                order = self.client.place_limit_order(self.symbol, 'sell', self.position_size, self.sell_level)
